"This file is a replacing pattern example with Python"
...
The r before a string indicates that the string is a raw string. A raw string is a
string literal that is not interpreted by the Python interpreter, so all characters
in the string are taken literally.

We use b only if it is a complete word (surrounded by word boundaries). Characters are word
boundary assertions.
...
re.sub() returns a new string with all occurrences of the pattern replaced
